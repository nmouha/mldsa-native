...
let mldsa_rej_uniform_eta2_mc = define_assert_from_elf
  "mldsa_rej_uniform_eta2_mc" "aarch64/mldsa/mldsa_rej_uniform_eta2.o"
(*** BYTECODE START ***)
[
...
(* Nibble extraction — reuse from eta4 if already loaded, else define here *)
(* These definitions are idempotent if eta4 has already been loaded *)
...
(* Rejection sampling spec for eta=2 *)
...
let REJ_SAMPLE_ETA2_EMPTY = prove
 (`REJ_SAMPLE_ETA2 [] = []`,
  REWRITE_TAC[REJ_SAMPLE_ETA2; REJ_NIBBLES_ETA2_EMPTY; MAP]);;
...
let REJ_SAMPLE_ETA2_APPEND = prove
 (`!l1 l2. REJ_SAMPLE_ETA2(APPEND l1 l2) =
           APPEND (REJ_SAMPLE_ETA2 l1) (REJ_SAMPLE_ETA2 l2)`,
  REWRITE_TAC[REJ_SAMPLE_ETA2; REJ_NIBBLES_ETA2_APPEND; MAP_APPEND]);;
...
(* Q31 word_insert simplification — same as eta4, same V31 setup *)
...
(* REJ_NIBBLES_ETA2 splits into two groups of 8 nibbles *)
let REJ_NIBBLES_ETA2_8 = prove(
  `!b0 b1 b2 b3 b4 b5 b6 b7:byte.
    REJ_NIBBLES_ETA2 [b0;b1;b2;b3;b4;b5;b6;b7] =
    APPEND
      (FILTER (\x:int16. val x < 15)
  ...
  REPEAT GEN_TAC THEN
  REWRITE_TAC[REJ_NIBBLES_ETA2; NIBBLES_OF_BYTES_8; FILTER_APPEND]);;
...
let MLDSA_REJ_UNIFORM_ETA2_CORRECT = prove
 (`!res buf buflen table (inlist:byte list) pc stackpointer.
  ...
  (* === Split: computation (pc+4 to pc+0xfc) and writeback (to pc+0x16c) *)
  ...
  CONJ_TAC THENL
   [ALL_TAC; (* writeback phase deferred to second conjunct *)
    ...
    (* ============================================================= *)
    (* WRITEBACK PHASE: from pc+0xfc to pc+0x16c                     *)
    (* CMP+CSEL to cap at 256, then Barrett mod-5 reduction with     *)
    (* SQDMULH+MLS, subtract from 2, sign-extend to 32-bit.         *)
    (* 16 iterations of 19-instruction copy loop.                    *)
    (* ============================================================= *)

    ENSURES_INIT_TAC "s0" THEN
    FIRST_X_ASSUM(X_CHOOSE_THEN `n:num` MP_TAC) THEN
    CONV_TAC(TOP_DEPTH_CONV let_CONV) THEN
    ABBREV_TAC `niblist = REJ_NIBBLES_ETA2 (SUB_LIST (0,8 * n) inlist)` THEN
    ...
    (* Writeback: 313 ARM steps. SQDMULH/MLS for Barrett mod-5 are *)
    (* expensive (~75s per 19-step copy iteration, 16 iterations). *)
    (* Total ~20 minutes on current hardware.                       *)
    (* Writeback: 313 ARM steps including Barrett mod-5 reduction   *)
    (* (SQDMULH+MLS). Total execution time ~20 minutes.            *)
    ARM_STEPS_TAC MLDSA_REJ_UNIFORM_ETA2_EXEC (1--313) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    CHEAT_TAC] THEN
    (* WRITEBACK POSTCONDITION - debugging: tryfind error above *)
    (* SUBGOAL_THEN
     `read (memory :> bytes (res,4 * MIN 256 niblen)) s313 =
      num_of_wordlist (SUB_LIST (0,256) (REJ_SAMPLE_ETA2
        (SUB_LIST(0,8 * n) inlist)):int32 list)`
    ASSUME_TAC THENL
     [REWRITE_TAC[REJ_SAMPLE_ETA2; NUM_OF_WORDLIST_SUB_LIST_0;
     ...
     [SUBGOAL_THEN `SUB_LIST (0,8 * n) (inlist:byte list) = inlist`
      ...
      CONJ_TAC THENL
       [REWRITE_TAC[ARITH_RULE `MIN a b = a <=> a <= b`] THEN
        REWRITE_TAC[REJ_SAMPLE_ETA2; LENGTH_MAP] THEN
        ...
        REWRITE_TAC[REJ_NIBBLES_ETA2_APPEND; LENGTH_APPEND; LE_ADD];
        DISCH_THEN SUBST1_TAC] THEN
      SUBGOAL_THEN
       `SUB_LIST (0,256) (REJ_SAMPLE_ETA2 inlist) =
        SUB_LIST (0,256) (REJ_SAMPLE_ETA2 (SUB_LIST(0,8*n) inlist))`
      SUBST1_TAC THENL
       [MP_TAC(ISPECL [`inlist:byte list`; `8 * n`]
          SUB_LIST_TOPSPLIT) THEN
        DISCH_THEN(fun th -> ONCE_REWRITE_TAC[SYM th]) THEN
        ASM_SIMP_TAC[REJ_SAMPLE_ETA2_APPEND; SUB_LIST_APPEND_LEFT] THEN
        REWRITE_TAC[REJ_SAMPLE_ETA2; LENGTH_MAP] THEN ASM_ARITH_TAC;
        ALL_TAC] THEN
  ...
  (* ================================================================= *)
  (* COMPUTATION PHASE: from pc+4 to pc+0xfc                           *)
  ...
  SUBGOAL_THEN
   `?i. buflen < 8 * (i + 1) \/
        256 <= LENGTH(REJ_NIBBLES_ETA2(SUB_LIST(0,8 * i) inlist))`
  MP_TAC THENL
  ...
  SUBGOAL_THEN `0 < N` ASSUME_TAC THENL
   [REWRITE_TAC[ARITH_RULE `0 < N <=> ~(N = 0)`] THEN
    ...
    REWRITE_TAC[MULT_CLAUSES; ADD_CLAUSES;
                SUB_LIST_CLAUSES; REJ_NIBBLES_ETA2_EMPTY; LENGTH] THEN
    UNDISCH_TAC `8 <= buflen` THEN ARITH_TAC;
    ALL_TAC] THEN

  (* --- Set up loop: ENSURES_WHILE_UP_TAC --- *)
  (* Loop head at pc+0x68 (entry 26: CMP X9,X4)                     *)
  (* Back edge at pc+0xf4 (entry 61: CMP X2,#8)                     *)
  (* NOTE: Q7 is NOT in the loop invariant for eta2 because it is   *)
  (* set only during the writeback setup (after the main loop).     *)
  ...
  REPEAT CONJ_TAC THENL
   [(*** Subgoal 1: 0 < N ***)
    ...
    (*** Subgoal 2: Pre-loop initialization — 74 ARM steps ***)
    (*** From pc+4 to pc+0x68: setup V30/V31 (NO V7), zero stack, init regs ***)
    ...
    (*** From pc+0x68 (invariant i) to pc+0xf4 (invariant i+1)       ***)
    ...
    ARM_STEPS_TAC MLDSA_REJ_UNIFORM_ETA2_EXEC (3--35) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[];
    (*** Subgoal 4: Back edge — 2 ARM steps from pc+0xf4 to pc+0x68 ***)
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN
    ...
    SUBGOAL_THEN
     `LENGTH (REJ_NIBBLES_ETA2 (SUB_LIST (0,8 * N) inlist)) < 272`
    ASSUME_TAC THENL
     [ASM_CASES_TAC `N = 0` THENL
      ...
      DISCH_THEN SUBST1_TAC THEN REWRITE_TAC[REJ_NIBBLES_ETA2_APPEND] THEN
      REWRITE_TAC[LENGTH_APPEND; LE_ADD_LCANCEL; ADD_CLAUSES] THEN
      TRANS_TAC LE_TRANS
       `2 * LENGTH(SUB_LIST(8 * (N-1),8) (inlist:byte list))` THEN
      CONJ_TAC THENL [REWRITE_TAC[LENGTH_REJ_NIBBLES_ETA2]; ALL_TAC] THEN
      REWRITE_TAC[LENGTH_SUB_LIST] THEN ARITH_TAC;
      ALL_TAC] THEN
    VAL_INT64_TAC
     `LENGTH (REJ_NIBBLES_ETA2 (SUB_LIST (0,8 * N) inlist))` THEN
...
let LENGTH_SIMPLIFY_CONV =
  REWRITE_CONV[LENGTH_MLDSA_REJ_UNIFORM_ETA2_MC] THENC
  NUM_REDUCE_CONV THENC REWRITE_CONV [ADD_0];;

(* ------------------------------------------------------------------------- *)
(* Subroutine form: includes stack frame allocation/deallocation and return.  *)
(* C signature: uint64_t mld_rej_uniform_eta2_asm(int32_t *r,                *)
